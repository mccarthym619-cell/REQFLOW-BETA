...
Dev role-switcher toolbar at top of page. Uses `X-Current-User-Id` header — defaults to user 1 (admin) on page refresh. Middleware at `server/src/middleware/auth.ts`. DevToolbar only renders in development.
...
- **File upload**: "Upload-first" pattern — `POST /api/files/upload` returns file ID, stored as string field value, linked to request on save. Download via `GET /api/files/:id/download`. Supports single-file (`file` type) and multi-file (`multi_file` type, stores JSON array of file IDs)
...
- **File uploads**: UUID filenames, 10MB limit, MIME whitelist
...
Toggleable dark mode via Sun/Moon icon in the top bar. Uses Tailwind `darkMode: 'class'` strategy.

- **ThemeContext** (`client/src/context/ThemeContext.tsx`): Manages `dark` class on `<html>`, persists to localStorage, exports `useTheme()` hook
...
```
shared/src/
  types/          — FieldType, CustomFieldDefinition, Request, User, Priority, etc.
...
server/src/
  ...
  utils/          — referenceNumber.ts (naming convention generation)

client/src/
  api/            — axios client (withCredentials, auth header, 401 interceptor)
  context/        — SessionContext (session auth), AuthContext (current user + role), ThemeContext (dark mode), NotificationContext (SSE)
  components/
    layout/       — Sidebar, TopBar (includes dark mode toggle), DevToolbar
    shared/       — StatusBadge, PriorityBadge, LoadingSpinner, EmptyState, FileUploadInput, MultiFileUploadInput, HelpGuide
  pages/
    Admin/        — Templates (list + builder), Users, AuditLog, Settings
    Approvals/    — PendingApprovalsPage
    Auth/         — LoginPage (email + password two-step)
    Dashboard/    — DashboardPage (role-aware cards)
    Notifications/
...
- `/api/users` — CRUD + current user + password reset
